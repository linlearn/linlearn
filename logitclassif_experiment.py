...
logging.info(
    "Parameters are : loss = %s , n_repeats = %d , n_samples = %d , max_iter = %d , fit_intercept=%r, test_loss_meantype = %s"
    % (loss, n_repeats, n_samples or 0, max_iter, fit_intercept, test_loss_meantype)
)
...
penalty = None  # "l1"#
lamda = 1.0  # /np.sqrt(X_train.shape[0])
...
def objective(X, y, w, fit_intercept=fit_intercept, lnlearn=False, meantype="ordinary"):
    objectives = sample_objectives(
        X, y, w, fit_intercept=fit_intercept, lnlearn=lnlearn
    )
...
def sample_gradients(X, y, w, fit_intercept=fit_intercept):
    ...
    if fit_intercept:
        return np.hstack(
            (derivatives[:, np.newaxis], np.einsum("i,ij->ij", derivatives, X))
        )
    else:
...
def train_loss(w, algo_name=""):
    return objective(
        ...
        lnlearn=algo_name in linlearn_algorithms,
    )
...
if os.cpu_count() > 8:
    ...
    results = joblib.Parallel(n_jobs=-1)(
        joblib.delayed(run_repetition)(rep) for rep in range(n_repeats)
    )
else:
...
g.fig.suptitle(
    "data : %s , n=%d, block_size=%.2f, loss=%s"
    % (dataset, n_samples, block_sizes["mom_cgd"], loss.upper())
)
