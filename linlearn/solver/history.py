...
from numba import jitclass, objmode
from numba.types import int64, boolean, List, float64
...
class History(object):
    ...
    def __init__(self, title, max_iter, verbose):
        ...
        bar_format = '{desc} : {percentage:2.0f}% {bar} epoch: {n_fmt} / {total_fmt} , ' \
                     'elapsed: {elapsed_s:3.1f}s {postfix}'
        self.bar = trange(max_iter, desc=title, unit=' epoch ', leave=True, bar_format=bar_format)

    def update(self, objective, tol):
        if self.verbose:
            s = ' , '.join([('obj: %.2e' % objective), ('tol: %.2e' % tol)])
            self.bar.set_postfix_str(s)
            self.bar.update(1)
    ...
    # TODO: code a print() history method ? or some magic method ?
